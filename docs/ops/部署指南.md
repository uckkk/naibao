# 服务器部署指南

## 🚀 快速部署

### 前提条件

- ✅ 服务器已准备：`<SSH_HOST>`
- ✅ 已安装SSH客户端
- ✅ 本地已安装Go（用于编译，可选）
- ✅ 已配置本机私有连接信息：`scripts/.env.local`（参考 `scripts/.env.example`）

---

## 📋 部署步骤

### 方法1：使用部署脚本（推荐）

```bash
# 1. 进入项目目录
cd "<你的项目目录>/naibao"

# 2. 运行部署脚本
./scripts/deploy.sh
```

脚本会自动：
- 上传代码到服务器
- 安装依赖
- 初始化数据库（可选）
- 编译和启动服务（可选）

---

### 方法2：手动部署

#### 步骤1：连接服务器

```bash
ssh -p <SSH_PORT> <SSH_USER>@<SSH_HOST>
```

#### 步骤2：初始化服务器（首次部署）

```bash
# 运行初始化脚本
bash <(curl -s https://raw.githubusercontent.com/your-repo/naibao/main/scripts/server_init.sh)

# 或手动执行
cd /opt/naibao
./scripts/server_init.sh
```

#### 步骤3：上传代码

在本地执行：

```bash
# 上传后端代码
scp -r backend <SSH_USER>@<SSH_HOST>:/opt/naibao/

# 上传数据库脚本
scp -r database <SSH_USER>@<SSH_HOST>:/opt/naibao/
```

#### 步骤4：配置环境变量

在服务器上：

```bash
# 进入后端目录
cd /opt/naibao/backend

# 创建.env文件
nano .env
```

配置内容：

```env
SERVER_PORT=8080
GIN_MODE=release

DB_HOST=localhost
DB_PORT=5432
DB_USER=naibao_user
DB_PASSWORD=你的数据库密码
DB_NAME=naibao
DB_SSLMODE=disable

REDIS_HOST=localhost
REDIS_PORT=6379
REDIS_PASSWORD=

JWT_SECRET=你的JWT密钥（至少32位）
```

#### 步骤5：初始化数据库

```bash
# 导入表结构
psql -U naibao_user -d naibao -f /opt/naibao/database/schema.sql

# 导入初始化数据
psql -U naibao_user -d naibao -f /opt/naibao/database/init_data.sql
```

#### 步骤6：编译和运行

```bash
# 进入后端目录
cd /opt/naibao/backend

# 安装依赖
go mod download

# 编译
go build -o naibao-server main.go

# 测试运行
./naibao-server
```

如果看到 `Server starting on :8080`，说明启动成功！

#### 步骤7：配置systemd服务（推荐）

```bash
# 创建服务文件
nano /etc/systemd/system/naibao.service
```

内容：

```ini
[Unit]
Description=Naibao Backend Server
After=network.target postgresql.service

[Service]
Type=simple
User=root
WorkingDirectory=/opt/naibao/backend
ExecStart=/opt/naibao/backend/naibao-server
Restart=always
RestartSec=10
Environment="GIN_MODE=release"

[Install]
WantedBy=multi-user.target
```

启动服务：

```bash
# 重载systemd
systemctl daemon-reload

# 启动服务
systemctl start naibao

# 设置开机自启
systemctl enable naibao

# 查看状态
systemctl status naibao
```

---

## 🔍 验证部署

### 1. 检查服务状态

```bash
# 检查后端服务
systemctl status naibao

# 检查PostgreSQL
systemctl status postgresql

# 检查Redis
systemctl status redis
```

### 2. 测试API

```bash
# 健康检查
curl http://localhost:8080/health

# 应该返回: {"status":"ok"}
```

### 3. 查看日志

```bash
# 后端服务日志
journalctl -u naibao -f

# 或查看文件日志
tail -f /opt/naibao/backend/logs/app.log
```

---

## 🔧 常用操作

### 重启服务

```bash
systemctl restart naibao
```

### 停止服务

```bash
systemctl stop naibao
```

### 查看日志

```bash
journalctl -u naibao -f
```

### 更新代码

```bash
# 1. 停止服务
systemctl stop naibao

# 2. 上传新代码
# (在本地执行)
scp -r backend <SSH_USER>@<SSH_HOST>:/opt/naibao/

# 3. 在服务器上重新编译
cd /opt/naibao/backend
go build -o naibao-server main.go

# 4. 启动服务
systemctl start naibao
```

---

## 🔒 安全建议

### 1. 修改默认密码

```bash
# 修改数据库密码
sudo -u postgres psql
ALTER USER naibao_user WITH PASSWORD '新密码';
\q
```

### 2. 配置防火墙

```bash
# 只允许必要的端口
ufw allow 22/tcp    # SSH
ufw allow 8080/tcp  # 后端API（如果需要外网访问）
ufw enable
```

### 3. 使用HTTPS（生产环境）

建议使用Nginx反向代理，配置SSL证书。

---

## 📝 故障排查

### 问题1：服务无法启动

```bash
# 查看详细错误
journalctl -u naibao -n 50

# 检查端口是否被占用
netstat -tulpn | grep 8080
```

### 问题2：数据库连接失败

```bash
# 检查PostgreSQL是否运行
systemctl status postgresql

# 测试连接
psql -U naibao_user -d naibao -c "SELECT 1;"
```

### 问题3：端口无法访问

```bash
# 检查防火墙
ufw status

# 检查服务是否监听
netstat -tulpn | grep 8080
```

---

## ✅ 部署检查清单

- [ ] 服务器初始化完成
- [ ] PostgreSQL安装并运行
- [ ] Redis安装并运行（可选）
- [ ] 代码已上传到服务器
- [ ] 环境变量已配置
- [ ] 数据库已初始化
- [ ] 服务已编译
- [ ] systemd服务已配置
- [ ] 服务已启动
- [ ] API测试通过
- [ ] 防火墙已配置

---

**部署完成后，可以通过 `http://<API_HOST>:8080` 访问API！** 🎉
